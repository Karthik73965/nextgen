    ...
    <div className='overflow-x-hidden'>
     ...
     <div className='h-[70px]'></div>
      <div className="container  mx-auto mt-4 p-4 w-[80vw] sm:w-auto min-h-screen border-2 border-black  rounded-lg">
        ...
        <div className="flex flex-row gap-y-5 sm:gap-y-0  flex-wrap justify-between mb-4">
          {/* Tabs */}
          <div className="flex">
            ...
            <Link to={`/discussions/id`}>
              <div key={discussion.id} className="flex flex-row justify-between items-center py-4 border-b border-gray-300">
                <div className="flex-grow">
                  <h3 className="sm:text-lg text-md font-semibold">{discussion.title}</h3>
                  <p className="text-sm text-gray-600">Created at: {discussion.createdDate} | Last Reply: {discussion.lastReplyDate}</p>it
                  <div className="flex mt-2">
                    {discussion.tags.map(tag => <span key={tag} className="text-sm text-white p-2 bg-gray-800 rounded-full sm:px-3  sm:py-3 mr-2">{tag}</span>)}
                  </div>
                </div>
