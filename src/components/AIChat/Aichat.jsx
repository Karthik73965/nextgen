...
export default function Aichat() {
  ...
  const [inputValue, setInputValue] = useState("");
    ...
    const [messages, setMessages] = useState([
      {
        text: "Hey Buddy Welcome to NexgenStudy. This is Luna, How can i help you in your acedamics",
        sender: 'server'
      }
    ]);
          ...
          {messages.map((msg, index) => (
            <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end  ' : 'justify-start'}`}>
              <div className={`rounded-2xl px-4 max-w-[60vw] text-wrap my-5 py-2  ${msg.sender === 'user' ? ' min-w-[30vw]  border-2 bg-green-100  border-black' : '  border-2 min-w-[60vw]  border-gray-700 bg-gray-100'}`}>
                {
                  msg.sender =='server' ? <div className='text-gradient  font-bold'>{" Nexgen:"}</div> : <div className='text-gradient font-bold '>{"You :"}</div>
                }
                {msg.text}
                {
